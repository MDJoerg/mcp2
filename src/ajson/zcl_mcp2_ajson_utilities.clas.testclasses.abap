...
class lcl_nodes_helper definition final.
  public section.
    ...
    data mt_nodes type zif_mcp2_ajson_types=>ty_nodes_tt read-only.
...
class ltcl_json_utils definition
  ...
  private section.
    ...
    methods json_diff for testing raising zcx_mcp2_ajson_error.
    methods json_diff_types for testing raising zcx_mcp2_ajson_error.
    methods json_diff_arrays for testing raising zcx_mcp2_ajson_error.
    methods json_merge for testing raising zcx_mcp2_ajson_error.
    methods json_sort for testing raising zcx_mcp2_ajson_error.
    methods is_equal for testing raising zcx_mcp2_ajson_error.

    methods iterate_array for testing raising zcx_mcp2_ajson_error.
    methods iterate_object for testing raising zcx_mcp2_ajson_error.
...
class ltcl_json_utils implementation.
  ...
  method json_diff.
    ...
    data:
      lv_json       type string,
      lo_util       type ref to zcl_mcp2_ajson_utilities,
      lo_insert     type ref to zif_mcp2_ajson,
      lo_delete     type ref to zif_mcp2_ajson,
      lo_change     type ref to zif_mcp2_ajson,
...
*      '        "row": 4,\n' && " array delete
  ...
  method json_merge.
    ...
    data:
      ...
      lv_json_b    type string,
      lo_util      type ref to zcl_mcp2_ajson_utilities,
      lo_merge     type ref to zif_mcp2_ajson,
  ...
  method is_equal.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = zcl_mcp2_ajson_utilities=>new( )->is_equal(
        ii_json_a = zcl_mcp2_ajson=>parse( '{"a":1,"b":2}' )
        ii_json_b = zcl_mcp2_ajson=>parse( '{"a":1,"b":2}' ) )
      exp = abap_true ).
  ...
  method iterate_array.
    ...
    data li_iterator type ref to zif_mcp2_ajson_iterator.
    ...
    data li_json_item type ref to zif_mcp2_ajson.

    li_iterator = zcl_mcp2_ajson_utilities=>iterate_array(
      ii_json = li_cut
      iv_path = '/b' ).
    ...
    try.
      zcl_mcp2_ajson_utilities=>iterate_array(
        ii_json = li_cut
        iv_path = '/notexisting' ).
  ...
  method iterate_object.
    ...
    li_iterator = zcl_mcp2_ajson_utilities=>iterate_object(
      ii_json = li_cut
      iv_path = '/b' ).
    while li_iterator->has_next( ) = abap_true.
      li_json_item = li_iterator->next( ).
    ...
    try.
      zcl_mcp2_ajson_utilities=>iterate_object(
        ii_json = li_cut
        iv_path = '/notexisting' ).
      cl_abap_unit_assert=>fail( ).
    catch zcx_mcp2_ajson_error into lx.
