class zcl_mcp2_ajson definition
  public
  create public.

  public section.

    interfaces zif_mcp2_ajson.

    aliases:
      is_empty for zif_mcp2_ajson~is_empty,
      exists for zif_mcp2_ajson~exists,
      members for zif_mcp2_ajson~members,
      get for zif_mcp2_ajson~get,
      get_node_type for zif_mcp2_ajson~get_node_type,
      get_boolean for zif_mcp2_ajson~get_boolean,
      get_integer for zif_mcp2_ajson~get_integer,
      get_number for zif_mcp2_ajson~get_number,
      get_date for zif_mcp2_ajson~get_date,
      get_timestamp for zif_mcp2_ajson~get_timestamp,
      get_string for zif_mcp2_ajson~get_string,
      slice for zif_mcp2_ajson~slice,
      to_abap for zif_mcp2_ajson~to_abap,
      array_to_string_table for zif_mcp2_ajson~array_to_string_table.

    aliases:
      clear for zif_mcp2_ajson~clear,
      set for zif_mcp2_ajson~set,
      setx for zif_mcp2_ajson~setx,
      set_boolean for zif_mcp2_ajson~set_boolean,
      set_string for zif_mcp2_ajson~set_string,
      set_integer for zif_mcp2_ajson~set_integer,
      set_date for zif_mcp2_ajson~set_date,
      set_timestamp for zif_mcp2_ajson~set_timestamp,
      set_null for zif_mcp2_ajson~set_null,
      delete for zif_mcp2_ajson~delete,
      touch_array for zif_mcp2_ajson~touch_array,
      push for zif_mcp2_ajson~push,
      stringify for zif_mcp2_ajson~stringify.

    aliases:
      clone for zif_mcp2_ajson~clone,
      filter for zif_mcp2_ajson~filter,
      map for zif_mcp2_ajson~map.

    aliases:
      mt_json_tree for zif_mcp2_ajson~mt_json_tree,
      keep_item_order for zif_mcp2_ajson~keep_item_order,
      format_datetime for zif_mcp2_ajson~format_datetime,
      to_abap_corresponding_only for zif_mcp2_ajson~to_abap_corresponding_only,
      freeze for zif_mcp2_ajson~freeze.
    ...
    class-methods parse
      ...
      returning
        value(ro_instance) type ref to zcl_mcp2_ajson
      raising
    ...
    class-methods create_from " TODO, rename to 'from' ?
      importing
        !ii_source_json type ref to zif_mcp2_ajson
        !ii_filter type ref to zif_mcp2_ajson_filter optional " Might be deprecated, use filter() instead
        !ii_mapper type ref to zif_mcp2_ajson_mapping optional " Might be deprecated, use map() instead
      returning
        value(ro_instance) type ref to zcl_mcp2_ajson
      raising
        zcx_mcp2_ajson_error.
  ...
  private section.
    ...
    data ms_opts type zif_mcp2_ajson=>ty_opts.
    data mi_custom_mapping type ref to zif_mcp2_ajson_mapping. " DEPRECATED, will be removed

    methods get_item
      importing
        iv_path        type string
      returning
        value(rv_item) type ref to zif_mcp2_ajson_types=>ty_node.
    methods prove_path_exists
      importing
        iv_path              type string
      returning
        value(rr_end_node) type ref to zif_mcp2_ajson_types=>ty_node
      raising
        zcx_mcp2_ajson_error.
    methods delete_subtree
      importing
        iv_path           type string
        iv_name           type string
        ir_parent         type ref to zif_mcp2_ajson_types=>ty_node optional
      returning
        value(rs_top_node) type zif_mcp2_ajson_types=>ty_node.
...
CLASS zcl_mcp2_ajson IMPLEMENTATION.
  ...
  method create_from.
    ...
    if ii_source_json is not bound.
      zcx_mcp2_ajson_error=>raise( 'Source not bound' ) ##NO_TEXT.
    endif.
  ...
  method delete_subtree.
    ...
    if rs_top_node-children > 0. " only for objects and arrays
      " mcp2 vendoring fix: TAB-escape the name - child paths store TAB for
      " literal '/' in member names
      lv_parent_path = iv_path && lcl_utils=>escape_name( iv_name ) && '/*'.
  ...
  method get_item.
    ...
    data ls_path_name type zif_mcp2_ajson_types=>ty_path_name.
  ...
  method prove_path_exists.
    ...
    data lv_cur_seg type string.
    data ls_new_node like line of mt_json_tree.

    split iv_path at '/' into table lt_path.
    delete lt_path where table_line is initial.

    " mcp2 vendoring fix: path segments may carry the TAB escape for literal
    " slashes in member names. Internal paths keep the TAB form (lv_cur_seg);
    " node names and lookups use the unescaped member name (lv_cur_name).
    do.
      ...
      if sy-subrc <> 0. " New node, assume it is always object as it has a named child, use touch_array to init array
        ...
        if lr_node_parent is not initial. " if has parent
          ...
          if lr_node_parent->type = zif_mcp2_ajson_types=>node_type-array.
        ...
        ls_new_node-type = zif_mcp2_ajson_types=>node_type-object.
        insert ls_new_node into table mt_json_tree reference into rr_end_node.
      endif.
      lv_cur_path = lv_cur_path && lv_cur_seg && '/'.
      read table lt_path index sy-index into lv_cur_seg.
      if sy-subrc <> 0.
        exit. " no more segments
      endif.
      lv_cur_name = replace( val  = lv_cur_seg
  ...
  method read_only_watchdog.
    if ms_opts-read_only = abap_true.
      zcx_mcp2_ajson_error=>raise( 'This json instance is read only' ) ##NO_TEXT.
    endif.
  endmethod.


  method zif_mcp2_ajson~array_to_string_table.

    data lv_normalized_path type string.
    data lr_node type ref to zif_mcp2_ajson_types=>ty_node.
    ...
    if lr_node is initial.
      zcx_mcp2_ajson_error=>raise( |Path not found: { iv_path }| ) ##NO_TEXT.
    endif.
    if lr_node->type <> zif_mcp2_ajson_types=>node_type-array.
      zcx_mcp2_ajson_error=>raise( |Array expected at: { iv_path }| ) ##NO_TEXT.
    endif.

    loop at mt_json_tree assigning <item> where path = lv_normalized_path.
      case <item>-type.
        when zif_mcp2_ajson_types=>node_type-number or zif_mcp2_ajson_types=>node_type-string.
        ...
        when others.
          zcx_mcp2_ajson_error=>raise( |Cannot convert [{ <item>-type
            }] to string at [{ <item>-path }{ <item>-name }]| ) ##NO_TEXT.
      endcase.
    endloop.
  ...
  method zif_mcp2_ajson~clone.
    ri_json = create_from( me ).
  endmethod.


  method zif_mcp2_ajson~delete.

    read_only_watchdog( ).

    data ls_split_path type zif_mcp2_ajson_types=>ty_path_name.
  ...
  method zif_mcp2_ajson~exists.
    rv_exists = boolc( get_item( iv_path ) is not initial ).
  endmethod.


  method zif_mcp2_ajson~filter.
    ri_json = create_from(
      ii_source_json = me
      ii_filter      = ii_filter ).
  endmethod.
  ...
  method zif_mcp2_ajson~format_datetime.
    ms_opts-format_datetime = iv_use_iso.
    ri_json = me.
  endmethod.


  method zif_mcp2_ajson~freeze.
    ms_opts-read_only = abap_true.
  endmethod.


  method zif_mcp2_ajson~get.

    data lr_item type ref to zif_mcp2_ajson_types=>ty_node.
  ...
  method zif_mcp2_ajson~get_boolean.

    data lr_item type ref to zif_mcp2_ajson_types=>ty_node.
    lr_item = get_item( iv_path ).
    if lr_item is initial or lr_item->type = zif_mcp2_ajson_types=>node_type-null.
      return.
    elseif lr_item->type = zif_mcp2_ajson_types=>node_type-boolean.
  ...
  method zif_mcp2_ajson~get_date.
    ...
    if lr_item is not initial and lr_item->type = zif_mcp2_ajson_types=>node_type-string.
  ...
  method zif_mcp2_ajson~get_integer.

    data lr_item type ref to zif_mcp2_ajson_types=>ty_node.
    lr_item = get_item( iv_path ).
    if lr_item is not initial and lr_item->type = zif_mcp2_ajson_types=>node_type-number.
      rv_value = lr_item->value.
    endif.
  ...
  method zif_mcp2_ajson~get_node_type.
  ...
  method zif_mcp2_ajson~get_number.
  ...
  method zif_mcp2_ajson~get_string.

    data lr_item type ref to zif_mcp2_ajson_types=>ty_node.
    lr_item = get_item( iv_path ).
    if lr_item is not initial and lr_item->type <> zif_mcp2_ajson_types=>node_type-null.
      rv_value = lr_item->value.
    endif.
  ...
  method zif_mcp2_ajson~get_timestamp.
  ...
  method zif_mcp2_ajson~get_timestampl.
  ...
  method zif_mcp2_ajson~is_empty.
    rv_yes = boolc( lines( mt_json_tree ) = 0 ).
  endmethod.


  method zif_mcp2_ajson~keep_item_order.
    ms_opts-keep_item_order = abap_true.
    ri_json = me.
  endmethod.


  method zif_mcp2_ajson~map.
    ri_json = create_from(
      ii_source_json = me
      ii_mapper      = ii_mapper ).
  endmethod.
  ...
  method zif_mcp2_ajson~opts.
    rs_opts = ms_opts.
  endmethod.


  method zif_mcp2_ajson~push.

    data lr_parent type ref to zif_mcp2_ajson_types=>ty_node.
    data lr_new_node type ref to zif_mcp2_ajson_types=>ty_node.
    ...
    if lr_parent is initial.
      zcx_mcp2_ajson_error=>raise( |Path [{ iv_path }] does not exist| ) ##NO_TEXT.
    endif.

    if lr_parent->type <> zif_mcp2_ajson_types=>node_type-array.
      zcx_mcp2_ajson_error=>raise( |Path [{ iv_path }] is not array| ) ##NO_TEXT.
    endif.

    data lt_new_nodes type zif_mcp2_ajson_types=>ty_nodes_tt.
    data ls_new_path type zif_mcp2_ajson_types=>ty_path_name.
  ...
  method zif_mcp2_ajson~set.

    data ls_split_path type zif_mcp2_ajson_types=>ty_path_name.
    data lr_parent type ref to zif_mcp2_ajson_types=>ty_node.
    data ls_deleted_node type zif_mcp2_ajson_types=>ty_node.
    data lv_item_order type zif_mcp2_ajson_types=>ty_node-order.
    ...
    if iv_node_type is not initial
      and iv_node_type <> zif_mcp2_ajson_types=>node_type-boolean and iv_node_type <> zif_mcp2_ajson_types=>node_type-null
      and iv_node_type <> zif_mcp2_ajson_types=>node_type-number and iv_node_type <> zif_mcp2_ajson_types=>node_type-string.
      zcx_mcp2_ajson_error=>raise( |Unexpected type { iv_node_type }| ) ##NO_TEXT.
    endif.
    ...
    " Most callers write several members below an already-created object.
    " Avoid walking and splitting the complete parent path again in that
    " common case; prove_path_exists remains the creation fallback.
    lr_parent = get_item( ls_split_path-path ).
    if lr_parent is initial.
    ...
    elseif lr_parent->type = zif_mcp2_ajson_types=>node_type-object
  ...
  method zif_mcp2_ajson~setx.
    ...
    if lv_val = 'null'.
      zif_mcp2_ajson~set_null( lv_path ).
    elseif lv_val = 'true'.
      zif_mcp2_ajson~set_boolean(
        iv_path = lv_path
        iv_val  = abap_true ).
    elseif lv_val = 'false'.
      zif_mcp2_ajson~set_boolean(
  ...
  method zif_mcp2_ajson~set_boolean.
  ...
  method zif_mcp2_ajson~set_date.
  ...
  method zif_mcp2_ajson~set_integer.
  ...
  method zif_mcp2_ajson~set_null.
  ...
  method zif_mcp2_ajson~set_string.
  ...
  method zif_mcp2_ajson~set_timestamp.
  ...
  method zif_mcp2_ajson~set_timestampl.
  ...
  method zif_mcp2_ajson~slice.
    ...
    data lo_section         type ref to zcl_mcp2_ajson.
    data ls_item            like line of mt_json_tree.
    data lv_normalized_path type string.
    data ls_path_parts      type zif_mcp2_ajson_types=>ty_path_name.
  ...
  method zif_mcp2_ajson~stringify.
  ...
  method zif_mcp2_ajson~touch_array.
    ...
    if ls_split_path is initial. " Assign root, exceptional processing
      ...
      ls_new_node-type = zif_mcp2_ajson_types=>node_type-array.
    ...
    elseif lr_node->type <> zif_mcp2_ajson_types=>node_type-array.
      zcx_mcp2_ajson_error=>raise( |Path [{ iv_path }] already used and is not array| ) ##NO_TEXT.
    endif.
  ...
  method zif_mcp2_ajson~to_abap.
    ...
    lo_to_abap->to_abap(
      exporting
        it_nodes    = zif_mcp2_ajson~mt_json_tree
      changing
  ...
  method zif_mcp2_ajson~to_abap_corresponding_only.
