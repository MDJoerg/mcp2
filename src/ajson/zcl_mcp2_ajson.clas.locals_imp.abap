  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS split_path
      IMPORTING
        iv_path             TYPE string
      RETURNING
        VALUE(rv_path_name) TYPE zif_mcp2_ajson_types=>ty_path_name.
    " mcp2 vendoring fix: internal node paths TAB-escape literal slashes in
    " member names (parser convention). Every place that builds a child path
    " from a node name must escape it the same way, or members named like
    " "io.modelcontextprotocol/related-task" lose their children between
    " parse( ) and stringify( ).
    CLASS-METHODS escape_name
      IMPORTING
        iv_name        TYPE string
      RETURNING
...
CLASS lcl_utils IMPLEMENTATION.
  ...
  METHOD validate_array_index.
    ...
    IF NOT iv_index CO '0123456789'.
      zcx_mcp2_ajson_error=>raise( |Cannot add non-numeric key [{ iv_index }] to array [{ iv_path }]| ) ##NO_TEXT.
    ENDIF.
    rv_index = iv_index.
    IF rv_index = 0.
      zcx_mcp2_ajson_error=>raise( |Cannot add zero key to array [{ iv_path }]| ) ##NO_TEXT.
    ENDIF.
    ...
    CASE lo_type->type_kind.
      ...
      WHEN lif_kind=>table.
        ...
        IF lo_table_type->table_kind <> cl_abap_tabledescr=>tablekind_std.
          zcx_mcp2_ajson_error=>raise( 'Unsupported type of input table (must be standard table)' ) ##NO_TEXT.
        ENDIF.
        TRY.
          ...
          CATCH cx_root.
            zcx_mcp2_ajson_error=>raise( 'Error converting input table (should be string_table)' ) ##NO_TEXT.
        ENDTRY.
      WHEN OTHERS.
        zcx_mcp2_ajson_error=>raise( 'Unsupported type of input (must be char, string, string_table, or xstring)' ) ##NO_TEXT.
    ENDCASE.
  ...
  METHOD any_to_string.
    ...
    CASE lo_type->type_kind.
      ...
      WHEN lif_kind=>table.
        ...
        TRY.
            ASSIGN iv_data TO <data>.
            rv_str = concat_lines_of( table = <data> sep = cl_abap_char_utilities=>newline ) ##NO_TEXT.
          CATCH cx_root.
            zcx_mcp2_ajson_error=>raise( 'Error converting input table (should be string_table)' ) ##NO_TEXT.
        ENDTRY.
      WHEN OTHERS.
  ...
  METHOD sanity_check.
    ...
    IF sy-subrc <> 0.
      zcx_mcp2_ajson_error=>raise(
        ...
        iv_location = 'Line 1, Offset 1' ) ##NO_TEXT.
    ENDIF.
...
CLASS lcl_json_parser DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    METHODS parse
      ...
      RETURNING
        VALUE(rt_json_tree) TYPE zif_mcp2_ajson_types=>ty_nodes_tt
      RAISING
        zcx_mcp2_ajson_error.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      ty_stack_tt TYPE STANDARD TABLE OF REF TO zif_mcp2_ajson_types=>ty_node.
    ...
    METHODS _parse
      ...
      RETURNING
        VALUE(rt_json_tree) TYPE zif_mcp2_ajson_types=>ty_nodes_tt
      RAISING
        zcx_mcp2_ajson_error cx_dynamic_check. " cx_sxml_error is not released on Steampunk #153
...
CLASS lcl_json_parser IMPLEMENTATION.
  ...
  METHOD _get_location.
    ...
    rv_location = |Line { lv_line }, Offset { lv_pos }| ##NO_TEXT.
      ...
      CASE lo_node->type.
        WHEN if_sxml_node=>co_nt_element_open.
          ...
          IF sy-subrc = 0.
            ...
            IF <item>-name IS INITIAL.
              raise( 'Node without name (maybe not JSON)' ) ##NO_TEXT.
            ENDIF.
          ENDIF.
        ...
        WHEN if_sxml_node=>co_nt_element_close.
          ...
          IF lo_close->qname-name <> lr_stack_top->type.
            raise( 'Unexpected closing node type' ) ##NO_TEXT.
          ENDIF.
        ...
        WHEN OTHERS.
          raise( 'Unexpected node type' ) ##NO_TEXT.
      ENDCASE.
    ENDDO.

    IF lines( mt_stack ) > 0.
      raise( 'Unexpected end of data' ) ##NO_TEXT.
    ENDIF.
...
CLASS lcl_json_serializer DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    CLASS-METHODS stringify
      IMPORTING
        it_json_tree          TYPE zif_mcp2_ajson_types=>ty_nodes_ts
  ...
  PRIVATE SECTION.
    ...
    DATA mt_json_tree TYPE zif_mcp2_ajson_types=>ty_nodes_ts.
    DATA mv_keep_item_order TYPE abap_bool.
    ...
    METHODS stringify_node
      IMPORTING
        is_node TYPE zif_mcp2_ajson_types=>ty_node
      RAISING
        zcx_mcp2_ajson_error.
...
CLASS lcl_json_serializer IMPLEMENTATION.
  ...
  METHOD stringify_node.
    ...
    CASE is_node-type.
      ...
      WHEN zif_mcp2_ajson_types=>node_type-boolean OR zif_mcp2_ajson_types=>node_type-number.
        lv_item = lv_item && is_node-value.
      WHEN zif_mcp2_ajson_types=>node_type-null.
        lv_item = lv_item && 'null'.
      WHEN OTHERS.
        zcx_mcp2_ajson_error=>raise(
          iv_msg = |Unexpected type [{ is_node-type }]|
          iv_location = is_node-path && is_node-name ) ##NO_TEXT.
    ENDCASE.

    IF mv_indent_step > 0
      AND ( is_node-type = zif_mcp2_ajson_types=>node_type-array OR is_node-type = zif_mcp2_ajson_types=>node_type-object )
    ...
    IF is_node-type = zif_mcp2_ajson_types=>node_type-array OR is_node-type = zif_mcp2_ajson_types=>node_type-object.
      DATA lv_children_path TYPE string.
      DATA lv_tail TYPE string.

      " mcp2 vendoring fix: TAB-escape the name so slash-named members find
      " their children (paths store TAB for literal '/', see lcl_utils).
      lv_children_path = is_node-path && lcl_utils=>escape_name( is_node-name ) && '/'. " for root: path = '' and name = '', so result is '/'
...
CLASS lcl_json_to_abap DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        ...
        !ii_custom_mapping TYPE REF TO zif_mcp2_ajson_mapping OPTIONAL
        !ii_refs_initiator TYPE REF TO zif_mcp2_ajson_ref_init OPTIONAL.

    METHODS to_abap
      IMPORTING
        it_nodes    TYPE zif_mcp2_ajson_types=>ty_nodes_ts
      CHANGING
    ...
    METHODS to_timestamp
      IMPORTING
        iv_value         TYPE zif_mcp2_ajson_types=>ty_node-value
      RETURNING
  ...
  PRIVATE SECTION.
    ...
    DATA mr_nodes TYPE REF TO zif_mcp2_ajson_types=>ty_nodes_ts.
    DATA mi_custom_mapping TYPE REF TO zif_mcp2_ajson_mapping.
    DATA mi_refs_initiator TYPE REF TO zif_mcp2_ajson_ref_init.
    ...
    METHODS get_node_type
      IMPORTING
        is_node             TYPE zif_mcp2_ajson_types=>ty_node OPTIONAL " Empty for root
        is_parent_type      TYPE ty_type_cache OPTIONAL
...
CLASS lcl_json_to_abap IMPLEMENTATION.
  ...
  METHOD get_node_type.
    ...
    IF sy-subrc <> 0.
      ...
      CASE is_parent_type-type_kind.
        ...
        WHEN lif_kind=>struct_flat OR lif_kind=>struct_deep.
          ...
          IF sy-subrc <> 0.
            IF mv_corresponding = abap_false.
              zcx_mcp2_ajson_error=>raise( |Path not found| ) ##NO_TEXT.
            ELSE.
              CLEAR rs_node_type.
              RETURN.
            ENDIF.
        ...
        WHEN OTHERS.
          zcx_mcp2_ajson_error=>raise( |Unexpected parent type| ) ##NO_TEXT.
      ENDCASE.
  ...
  METHOD get_data_ref.
    ...
    IF mi_refs_initiator IS INITIAL.
      zcx_mcp2_ajson_error=>raise( 'Missing ref initiator' ) ##NO_TEXT.
    ENDIF.

    ro_ref = mi_refs_initiator->get_data_ref( is_node ).

    IF ro_ref IS INITIAL.
      zcx_mcp2_ajson_error=>raise( 'Cannot use initial data ref' ) ##NO_TEXT.
    ENDIF.
  ...
  METHOD any_to_abap.
    ...
    DATA lx_ajson TYPE REF TO zcx_mcp2_ajson_error.
    DATA lx_root TYPE REF TO cx_root.
    DATA lr_target_field TYPE REF TO data.

    FIELD-SYMBOLS <n> TYPE zif_mcp2_ajson_types=>ty_node.
    ...
    TRY.
        ...
        LOOP AT mr_nodes->* ASSIGNING <n> USING KEY array_index WHERE path = iv_path.
          ...
          IF ls_node_type-type_kind = lif_kind=>object_ref.
            ...
            zcx_mcp2_ajson_error=>raise( 'Cannot assign to ref' ) ##NO_TEXT.
          ENDIF.
          ...
          CASE is_parent_type-type_kind.
            WHEN lif_kind=>table.
              IF NOT ls_node_type-target_field_name CO '0123456789'.
                ...
                zcx_mcp2_ajson_error=>raise( 'Need index to access tables' ) ##NO_TEXT.
              ENDIF.
          ...
          CASE <n>-type.
            WHEN zif_mcp2_ajson_types=>node_type-object.
              IF ls_node_type-type_kind <> lif_kind=>struct_flat AND
                ...
                zcx_mcp2_ajson_error=>raise( 'Expected structure' ) ##NO_TEXT.
              ENDIF.
              any_to_abap(
            ...
            WHEN zif_mcp2_ajson_types=>node_type-array.
              IF NOT ls_node_type-type_kind = lif_kind=>table.
                zcx_mcp2_ajson_error=>raise( 'Expected table' ) ##NO_TEXT.
              ENDIF.
              any_to_abap(
                iv_path         = <n>-path && <n>-name && '/'
                is_parent_type  = ls_node_type
          ...
          IF is_parent_type-tab_item_buf IS BOUND. " Indirect hint that table was sorted/hashed, see get_node_type.
            TRY.
                ...
                IF sy-subrc <> 0.
                  zcx_mcp2_ajson_error=>raise( 'Duplicate insertion' ) ##NO_TEXT.
                ENDIF.
      ...
      CATCH zcx_mcp2_ajson_error INTO lx_ajson.
      ...
      CATCH cx_sy_conversion_no_number.
        zcx_mcp2_ajson_error=>raise(
          ...
          iv_location = <n>-path && <n>-name ) ##NO_TEXT.
  ...
  METHOD value_to_abap.
    ...
    IF is_node_type-type_kind CA lif_kind=>deep_targets.
      zcx_mcp2_ajson_error=>raise( |Unsupported target for value [{ is_node_type-type_kind }]| ) ##NO_TEXT.
    ENDIF.
    ...
    CASE is_node-type.
      WHEN zif_mcp2_ajson_types=>node_type-null.
        " Do nothing
      WHEN zif_mcp2_ajson_types=>node_type-boolean.
        " TODO: check type ?
      ...
      WHEN zif_mcp2_ajson_types=>node_type-string.
        ...
        ENDIF.
      WHEN OTHERS.
        zcx_mcp2_ajson_error=>raise( |Unexpected JSON type [{ is_node-type }]| ) ##NO_TEXT.
    ENDCASE.
  ...
  METHOD to_date.
    ...
    IF sy-subrc <> 0.
      zcx_mcp2_ajson_error=>raise( 'Unexpected date format' ) ##NO_TEXT.
    ENDIF.
  ...
  METHOD to_timestamp.
    ...
    IF lv_frac_part CA '123456789'.
      zcx_mcp2_ajson_error=>raise( 'Unexpected timestamp format' ) ##NO_TEXT.
    ENDIF.
  ...
  METHOD to_timestampl.
    ...
    TRY.
      ...
      CATCH cx_parameter_invalid_range cx_parameter_invalid_type.
        zcx_mcp2_ajson_error=>raise( 'Unexpected error calculating timestamp' ) ##NO_TEXT.
    ENDTRY.
  ...
  METHOD to_time.
    ...
    IF sy-subrc <> 0.
      zcx_mcp2_ajson_error=>raise( 'Unexpected time format' ) ##NO_TEXT.
    ENDIF.
...
CLASS lcl_abap_to_json DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS convert
      IMPORTING
        iv_data           TYPE any
        is_prefix         TYPE zif_mcp2_ajson_types=>ty_path_name OPTIONAL
        iv_array_index    TYPE i DEFAULT 0
        ii_custom_mapping TYPE REF TO zif_mcp2_ajson_mapping OPTIONAL
        is_opts           TYPE zif_mcp2_ajson=>ty_opts OPTIONAL
        iv_item_order     TYPE i DEFAULT 0
      RETURNING
        VALUE(rt_nodes)   TYPE zif_mcp2_ajson_types=>ty_nodes_tt
      RAISING
        zcx_mcp2_ajson_error.
    ...
    CLASS-METHODS insert_with_type
      IMPORTING
        iv_data           TYPE any
        iv_type           TYPE zif_mcp2_ajson_types=>ty_node_type
        is_prefix         TYPE zif_mcp2_ajson_types=>ty_path_name OPTIONAL
        iv_array_index    TYPE i DEFAULT 0
        ii_custom_mapping TYPE REF TO zif_mcp2_ajson_mapping OPTIONAL
        is_opts           TYPE zif_mcp2_ajson=>ty_opts OPTIONAL
  ...
  PRIVATE SECTION.
    ...
    METHODS convert_any
      IMPORTING
        ...
        is_prefix     TYPE zif_mcp2_ajson_types=>ty_path_name
        iv_index      TYPE i DEFAULT 0
        iv_item_order TYPE i DEFAULT 0
      CHANGING
        ct_nodes      TYPE zif_mcp2_ajson_types=>ty_nodes_tt
      RAISING
        zcx_mcp2_ajson_error.

    METHODS convert_ajson
      IMPORTING
        io_json       TYPE REF TO zif_mcp2_ajson
        is_prefix     TYPE zif_mcp2_ajson_types=>ty_path_name
        iv_index      TYPE i DEFAULT 0
        iv_item_order TYPE i DEFAULT 0
      CHANGING
...
CLASS lcl_abap_to_json IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    DATA lo_dummy TYPE REF TO zcl_mcp2_ajson.
        ...
        ELSEIF io_type->type_kind = lif_kind=>object_ref
          ...
          convert_ajson(
            ...
            CHANGING
              ct_nodes = ct_nodes ).
        ELSE.
          zcx_mcp2_ajson_error=>raise( |Unsupported type [{ io_type->type_kind
            }] @{ is_prefix-path && is_prefix-name }| ) ##NO_TEXT.
        ENDIF.
  ...
  METHOD convert_ajson.
    ...
    LOOP AT io_json->mt_json_tree ASSIGNING <src>.
      ...
      ELSE.
        " mcp2 vendoring fix: TAB-escape the prefix name in child paths
        <dst>-path = is_prefix-path && lcl_utils=>escape_name( is_prefix-name ) && <dst>-path.
      ENDIF.
    ENDLOOP.
  ...
  METHOD convert_value.
    ...
    IF io_type->absolute_name = '\TYPE-POOL=ABAP\TYPE=ABAP_BOOL'
      ...
      ls_node-type = zif_mcp2_ajson_types=>node_type-boolean.
    ...
    ELSEIF io_type->absolute_name = '\TYPE=TIMESTAMP'.
      IF mv_format_datetime = abap_true.
        ls_node-type  = zif_mcp2_ajson_types=>node_type-string.
        ls_node-value = format_timestamp( iv_data ).
      ELSE.
        ls_node-type  = zif_mcp2_ajson_types=>node_type-number.
    ...
    ELSEIF io_type->type_kind CO lif_kind=>numeric.
      ...
      ls_node-value = |{ iv_data }|.
    ELSE.
      zcx_mcp2_ajson_error=>raise( |Unexpected elementary type [{
        io_type->type_kind }] @{ is_prefix-path && is_prefix-name }| ) ##NO_TEXT.
    ENDIF.
  ...
  METHOD convert_ref.
    ...
    IF iv_data IS INITIAL.
      ls_node-type  = zif_mcp2_ajson_types=>node_type-null.
      ls_node-value = 'null'.
      APPEND ls_node TO ct_nodes.
    ELSE.
      ASSIGN iv_data->* TO <data>.
  ...
  METHOD convert_struc.
    ...
    ls_root-type  = zif_mcp2_ajson_types=>node_type-object.
    ...
    ls_next_prefix-path = is_prefix-path && lcl_utils=>escape_name( <root>-name ) && '/'. " mcp2 vendoring fix
  ...
  METHOD convert_table.
    ...
    ls_root-path  = is_prefix-path.
    ls_root-name  = is_prefix-name.
    ls_root-type  = zif_mcp2_ajson_types=>node_type-array.
    ls_root-index = iv_index.
    ls_root-order = iv_item_order.
  ...
  METHOD insert_value_with_type.
    ...
    IF io_type->type_kind CO lif_kind=>texts OR
      ...
      IF iv_type = zif_mcp2_ajson_types=>node_type-boolean AND iv_data <> 'true' AND iv_data <> 'false'.
        zcx_mcp2_ajson_error=>raise( |Unexpected boolean value [{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ELSEIF iv_type = zif_mcp2_ajson_types=>node_type-null AND iv_data IS NOT INITIAL.
        zcx_mcp2_ajson_error=>raise( |Unexpected null value [{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ELSEIF iv_type = zif_mcp2_ajson_types=>node_type-number AND iv_data CN '0123456789. E+-'.
        zcx_mcp2_ajson_error=>raise( |Unexpected numeric value [{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ELSEIF iv_type <> zif_mcp2_ajson_types=>node_type-string AND iv_type <> zif_mcp2_ajson_types=>node_type-boolean
        AND iv_type <> zif_mcp2_ajson_types=>node_type-null AND iv_type <> zif_mcp2_ajson_types=>node_type-number.
        zcx_mcp2_ajson_error=>raise( |Unexpected type for value [{ iv_type },{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ENDIF.
    ELSEIF io_type->type_kind CO lif_kind=>numeric.
      IF iv_type <> zif_mcp2_ajson_types=>node_type-number.
        zcx_mcp2_ajson_error=>raise( |Unexpected value for numeric [{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ENDIF.
    ELSE.
      zcx_mcp2_ajson_error=>raise( |Unexpected type [{ io_type->type_kind }] @{ lv_prefix }| ) ##NO_TEXT.
    ENDIF.
...
INTERFACE lif_mutator_runner.
  METHODS run
    IMPORTING
      it_source_tree TYPE zif_mcp2_ajson_types=>ty_nodes_ts
    EXPORTING
      et_dest_tree   TYPE zif_mcp2_ajson_types=>ty_nodes_ts
    RAISING
      zcx_mcp2_ajson_error.
ENDINTERFACE.
...
CLASS lcl_filter_runner DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS new
      IMPORTING
        ii_filter          TYPE REF TO zif_mcp2_ajson_filter
      RETURNING
  ...
  PRIVATE SECTION.
    DATA mi_filter TYPE REF TO zif_mcp2_ajson_filter.
    DATA mr_source_tree TYPE REF TO zif_mcp2_ajson_types=>ty_nodes_ts.
    DATA mr_dest_tree TYPE REF TO zif_mcp2_ajson_types=>ty_nodes_ts.

    METHODS walk
      IMPORTING
        iv_path   TYPE string
      CHANGING
        cs_parent TYPE zif_mcp2_ajson_types=>ty_node OPTIONAL
      RAISING
        zcx_mcp2_ajson_error.
...
CLASS lcl_filter_runner IMPLEMENTATION.
  ...
  METHOD walk.
    ...
    DATA ls_node TYPE zif_mcp2_ajson_types=>ty_node.
    DATA lv_tab_key TYPE string.

    IF cs_parent-type = zif_mcp2_ajson_types=>node_type-array.
      lv_tab_key = 'array_index'. " path + index
    ENDIF.

    LOOP AT mr_source_tree->* INTO ls_node USING KEY (lv_tab_key) WHERE path = iv_path.
      CASE ls_node-type.
        WHEN zif_mcp2_ajson_types=>node_type-boolean OR zif_mcp2_ajson_types=>node_type-null
          OR zif_mcp2_ajson_types=>node_type-number OR zif_mcp2_ajson_types=>node_type-string.

          IF mi_filter->keep_node( ls_node ) = abap_false.
            CONTINUE.
          ENDIF.

        WHEN zif_mcp2_ajson_types=>node_type-array OR zif_mcp2_ajson_types=>node_type-object.

          IF mi_filter->keep_node(
              is_node  = ls_node
              iv_visit = zif_mcp2_ajson_filter=>visit_type-open ) = abap_false.
            CONTINUE.
          ENDIF.
          ...
          IF mi_filter->keep_node(
              is_node  = ls_node
              iv_visit = zif_mcp2_ajson_filter=>visit_type-close ) = abap_false.
            CONTINUE.
          ENDIF.

        WHEN OTHERS.
          zcx_mcp2_ajson_error=>raise( |Unexpected node type { ls_node-type }| ) ##NO_TEXT.
      ENDCASE.
...
CLASS lcl_mapper_runner DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS new
      IMPORTING
        ii_mapper          TYPE REF TO zif_mcp2_ajson_mapping
      RETURNING
  ...
  PRIVATE SECTION.
    DATA mi_mapper TYPE REF TO zif_mcp2_ajson_mapping.
    DATA mr_source_tree TYPE REF TO zif_mcp2_ajson_types=>ty_nodes_ts.
    DATA mr_dest_tree TYPE REF TO zif_mcp2_ajson_types=>ty_nodes_ts.
    ...
    METHODS process_deep_node
      IMPORTING
        ...
        iv_node_type    TYPE zif_mcp2_ajson_types=>ty_node-type
      RAISING
        zcx_mcp2_ajson_error.
...
CLASS lcl_mapper_runner IMPLEMENTATION.
  ...
  METHOD lif_mutator_runner~run.
    ...
    IF sy-subrc <> 0
      OR NOT ( <root>-type = zif_mcp2_ajson_types=>node_type-array OR <root>-type = zif_mcp2_ajson_types=>node_type-object ).
  ...
  METHOD process_deep_node.
    ...
    LOOP AT mr_source_tree->* ASSIGNING <item> WHERE path = iv_path.
      ...
      IF iv_node_type <> zif_mcp2_ajson_types=>node_type-array.
      ...
      IF sy-subrc <> 0. " = 4 ?
        zcx_mcp2_ajson_error=>raise(
          ...
          is_node = ls_renamed_node ) ##NO_TEXT.
      ENDIF.

      " maybe also catch CX_SY_ITAB_DUPLICATE_KEY but secondary keys are not changed here, so not for now

      IF <item>-type = zif_mcp2_ajson_types=>node_type-array OR <item>-type = zif_mcp2_ajson_types=>node_type-object.
        process_deep_node(
...
CLASS lcl_mutator_queue IMPLEMENTATION.
  ...
  METHOD lif_mutator_runner~run.
    ...
    DATA lr_buf TYPE REF TO zif_mcp2_ajson_types=>ty_nodes_ts.
