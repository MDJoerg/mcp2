...
"! (IF_HTTP_REQUEST), which is not constructible in the Phase-5 JS-transpiled
"! test runtime (standalone CL_HTTP_REQUEST relies on a kernel SYSTEM-CALL) -
...
CLASS ltcl_http_req_icf DEFINITION FINAL
...
CLASS ltcl_http_req_icf IMPLEMENTATION.
  METHOD test_create_wraps.
    DATA icf_request TYPE REF TO if_http_request.
    DATA(result) = zcl_mcp2_http_req_icf=>create( icf_request ).
