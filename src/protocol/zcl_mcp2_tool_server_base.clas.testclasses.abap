...
CLASS ltcl_catalog_server IMPLEMENTATION.
  ...
  METHOD define_tools.
    ...
    DATA(schema) = NEW zcl_mcp2_schema_builder(
      )->add_string( name = `message` required = abap_true
      )->to_json( ).
...
CLASS ltcl_novalidate_server DEFINITION FINAL
  INHERITING FROM zcl_mcp2_tool_server_base.
  PUBLIC SECTION.
    METHODS zif_mcp2_server~get_name REDEFINITION.
    METHODS zif_mcp2_server~get_version REDEFINITION.
...
CLASS ltcl_novalidate_server IMPLEMENTATION.
  ...
  METHOD define_tools.
    result = VALUE #( ( name = `noop` title = `Noop`
...
CLASS ltcl_raising_server DEFINITION FINAL
  INHERITING FROM zcl_mcp2_tool_server_base.
  PUBLIC SECTION.
    METHODS zif_mcp2_server~get_name REDEFINITION.
    METHODS zif_mcp2_server~get_version REDEFINITION.
...
CLASS ltcl_tool_server_base DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS test_define_tools_raises FOR TESTING.
ENDCLASS.
...
CLASS ltcl_tool_server_base IMPLEMENTATION.
  ...
  METHOD test_tools_list.
    DATA(server) = NEW ltcl_catalog_server( ).
    DATA(result) = server->zif_mcp2_server~tools_list( NEW zcl_mcp2_req_list_tools(
  ...
  METHOD test_unknown_tool.
    DATA(server) = NEW ltcl_catalog_server( ).
    DATA(request) = NEW zcl_mcp2_req_call_tool(
      zcl_mcp2_ajson=>parse( `{"name":"missing","arguments":{}}` ) ).
  ...
  METHOD test_declared_tool_dispatch.
    DATA(server) = NEW ltcl_catalog_server( ).
    DATA(request) = NEW zcl_mcp2_req_call_tool(
      zcl_mcp2_ajson=>parse( `{"name":"echo","arguments":{"message":"hi"}}` ) ).
  ...
  METHOD test_catalog_built_once.
    ...
    DATA(server) = NEW ltcl_catalog_server( ).
    server->zif_mcp2_server~tools_list( NEW zcl_mcp2_req_list_tools(
      zcl_mcp2_ajson=>parse( `{}` ) ) ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = 1 act = server->define_tools_calls ).
  ENDMETHOD.
