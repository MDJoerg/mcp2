CLASS ltcl_req_list_prompts DEFINITION FINAL FOR TESTING
...
CLASS ltcl_req_list_prompts IMPLEMENTATION.
  METHOD test_no_cursor.
    DATA(req) = NEW zcl_mcp2_req_list_prompts( zcl_mcp2_ajson=>parse( `{}` ) ).
    cl_abap_unit_assert=>assert_false( req->has_cursor( ) ).
    cl_abap_unit_assert=>assert_initial( req->get_cursor( ) ).
  ENDMETHOD.

  METHOD test_with_cursor.
    DATA(req) = NEW zcl_mcp2_req_list_prompts( zcl_mcp2_ajson=>parse( `{"cursor":"next"}` ) ).
    cl_abap_unit_assert=>assert_true( req->has_cursor( ) ).
    cl_abap_unit_assert=>assert_equals( exp = `next`
                                        act = req->get_cursor( ) ).
  ENDMETHOD.
  ...
  METHOD test_with_meta.
    DATA(req) = NEW zcl_mcp2_req_list_prompts( zcl_mcp2_ajson=>parse( `{"_meta":{"token":"abc"}}` ) ).
